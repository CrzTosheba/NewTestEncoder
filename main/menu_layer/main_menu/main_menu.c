...
// Глобальные указатели на экраны (доступны из других файлов)
lv_obj_t *screens[6] = {NULL};

// Структура элемента меню
typedef struct {
    const char *label_text;     // Текст элемента
    const char *sensor_text;    // Дополнительный текст (сенсор)
    const void *img_src;        // Основная иконка
    const void *status_img_src; // Статусная иконка
} MenuItem;
 ...
 * 
 * @param cont Контейнер меню
 * @param cursor_index Индекс выбранного элемента
 */
static void highlight_box(lv_obj_t *cont, uint32_t cursor_index) {
...
/**
 * @brief Обработчик событий энкодера для главного меню
 * 
...
/**
 * @brief Создание элемента меню
 * 
 * @param cont Контейнер для элемента
 * @param item Данные элемента меню
 */
static void create_menu_item(lv_obj_t *cont, const MenuItem *item) {
