...
static const CoPumpsMenuItem co_pumps_menu_items[] = {
    ...
    {"Пауза перед старт, с", NULL, 1},                   // N-BeforeStartPause (int)
    {"Пауза перед стоп, с", NULL, 2},                   // N-BeforeStopPause (int)
    {"Пауза переключ., с", NULL, 3},                    // N-ChangeOverPause (int)
    {"Режим переключения", NULL, 4},                    // N-ChangeMode (enum)
    {"Период работы, ч", NULL, 5},                       // N-ChangeWHours (int)
    {"Период работы, сут", NULL, 6},                    // N-ChangeWDays (int)
    {"Время переключ., ч", NULL, 7},                    // N-ChangeHours (int)
    {"Время переключ., мин", NULL, 8},                   // N-ChangeMinutes (int)
    {"Сброс.наработку Н1", NULL, 9},                    // N1-ResetWHours (enum)
    {"Время наработки Н1, ч", NULL, 10},                  // N1-WHours (int)
    {"Кол-во запусков Н1", NULL, 11},                    // N1-WStarts (int)
    {"Сброс.наработку Н2", NULL, 12},                    // N2-ResetWHours (enum)
    {"Время наработки Н2, ч", NULL, 13},                  // N2-WHours (int)
    {"Кол-во запусков Н2", NULL, 14},                    // N2-WStarts (int)
    {"Тренировать", NULL, 15},                           // N-Training-En (enum)
    {"Период тренировки, c", NULL, 16},                  // N-Training-Period (int)
};
...
static void update_param_display(int param_index) {
    ...
    if (param_index == 1 || param_index == 2 || param_index == 3) {
        // Пауза перед старт, Пауза перед стоп, Пауза переключ. - секунды
        snprintf(full_str, sizeof(full_str), "%s с", value_str);
    } else if (param_index == 5 || param_index == 7 || param_index == 10 || param_index == 13) {
