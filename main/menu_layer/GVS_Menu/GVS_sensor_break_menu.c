...
static const char *TAG = "GVS_SENSOR_BREAK_MENU";

static void update_param_display(int param_index);
static const char* get_sensor_alarm_rtype_string(gvs_sensor_alarm_rtype_t value);

typedef struct {
    const char *label_text;
    const void *img_src;
    int param_index;
} GvsSensorBreakMenuItem;

static const GvsSensorBreakMenuItem gvs_sensor_break_menu_items[] = {
    {"                                Назад", &lv_im_arrow_right, -1},
    {"Авария д. Тпод_ГВС", NULL, 0},           // GVS_T1_EnAlarm (enum)
    {"Задержка, с", NULL, 1},                   // GVS_AIAlarmDelay (int)
    {"Сброс а. Тпод_ГВС", NULL, 2},            // GVS_T1_AlarmRType (enum)
};

lv_obj_t *gvs_sensor_break_cont = NULL;
static bool gvs_sensor_break_menu_initialized = false;
static bool gvs_sensor_break_menu_creation_in_progress = false;
static lv_obj_t *gvs_sensor_break_mask = NULL;
...
static gvs_sensor_alarm_en_t temp_GVS_T1_EnAlarm = GVS_SENSOR_ALARM_NO;
static int temp_GVS_AIAlarmDelay = 0;
static gvs_sensor_alarm_rtype_t temp_GVS_T1_AlarmRType = GVS_SENSOR_ALARM_RTYPE_MANUAL_3;
...
static const char* get_sensor_alarm_rtype_string(gvs_sensor_alarm_rtype_t value) {
    switch(value) {
        case GVS_SENSOR_ALARM_RTYPE_AUTO: return "АВТО";
        case GVS_SENSOR_ALARM_RTYPE_MANUAL: return "РУЧН";
        case GVS_SENSOR_ALARM_RTYPE_MANUAL_1: return "РУЧН-1";
        case GVS_SENSOR_ALARM_RTYPE_MANUAL_2: return "РУЧН-2";
        case GVS_SENSOR_ALARM_RTYPE_MANUAL_3: return "РУЧН-3";
        case GVS_SENSOR_ALARM_RTYPE_MANUAL_4: return "РУЧН-4";
        case GVS_SENSOR_ALARM_RTYPE_MANUAL_5: return "РУЧН-5";
        case GVS_SENSOR_ALARM_RTYPE_MANUAL_6: return "РУЧН-6";
        case GVS_SENSOR_ALARM_RTYPE_MANUAL_7: return "РУЧН-7";
        case GVS_SENSOR_ALARM_RTYPE_MANUAL_8: return "РУЧН-8";
        case GVS_SENSOR_ALARM_RTYPE_MANUAL_9: return "РУЧН-9";
        case GVS_SENSOR_ALARM_RTYPE_MANUAL_10: return "РУЧН-10";
        default: return "???";
    }
}

static void gvs_sensor_break_highlight_box(lv_obj_t *cont, uint32_t cursor_index) {
    ...
    for (uint32_t i = 0; i < child_cnt; i++) {
        ...
        bool is_editing_this = (edit_mode && editing_param_index == gvs_sensor_break_menu_items[i].param_index);
...
static void update_param_display(int param_index) {
    ...
    if (edit_mode && editing_param_index == param_index) {
        ...
        if (param_index == 0) {
            ...
            // Сброс а. Тпод_ГВС - enum АВТО/РУЧН/РУЧН-1/.../РУЧН-10
            snprintf(value_str, sizeof(value_str), "%s", 
                     get_sensor_alarm_rtype_string((gvs_sensor_alarm_rtype_t)editing_int_value));
        }
    } else {
        // Обычный режим
        if (param_index == 0) {
            snprintf(value_str, sizeof(value_str), "%s", 
                     GVS_T1_EnAlarm == GVS_SENSOR_ALARM_NO ? "НЕТ" : "ДА");
        } else if (param_index == 1) {
            snprintf(value_str, sizeof(value_str), "%d", GVS_AIAlarmDelay);
        } else if (param_index == 2) {
            snprintf(value_str, sizeof(value_str), "%s", get_sensor_alarm_rtype_string(GVS_T1_AlarmRType));
        }
    }
...
static void save_param_changes(void) {
    ...
    switch(editing_param_index) {
        case 0: GVS_T1_EnAlarm = (gvs_sensor_alarm_en_t)editing_int_value; break;
        case 1: GVS_AIAlarmDelay = editing_int_value; break;
        case 2: GVS_T1_AlarmRType = (gvs_sensor_alarm_rtype_t)editing_int_value; break;
    }
    ...
    menu_state_t *menu_state = get_menu_state(MENU_TYPE_GVS_ALARMS_SENSOR_BREAK);
    if (menu_state) {
        gvs_sensor_break_highlight_box(gvs_sensor_break_cont, menu_state->cursor_index);
    }
}
...
static void cancel_param_changes(void) {
    ...
    switch(editing_param_index) {
        case 0: editing_int_value = (int)temp_GVS_T1_EnAlarm; break;
        case 1: editing_int_value = temp_GVS_AIAlarmDelay; break;
        case 2: editing_int_value = (int)temp_GVS_T1_AlarmRType; break;
    }
...
static void enter_edit_mode(int param_index) {
    ...
    switch(param_index) {
        case 0:
            temp_GVS_T1_EnAlarm = GVS_T1_EnAlarm;
            editing_int_value = (int)GVS_T1_EnAlarm;
            break;
        case 1:
            temp_GVS_AIAlarmDelay = GVS_AIAlarmDelay;
            editing_int_value = GVS_AIAlarmDelay;
            break;
        case 2:
            temp_GVS_T1_AlarmRType = GVS_T1_AlarmRType;
            editing_int_value = (int)GVS_T1_AlarmRType;
            break;
    }
...
static void exit_edit_mode_with_confirmation(void) {
    ...
    switch(editing_param_index) {
        case 0: value_changed = (editing_int_value != (int)temp_GVS_T1_EnAlarm); break;
        case 1: value_changed = (editing_int_value != temp_GVS_AIAlarmDelay); break;
        case 2: value_changed = (editing_int_value != (int)temp_GVS_T1_AlarmRType); break;
    }
...
static void create_gvs_sensor_break_menu_item(lv_obj_t *cont, const GvsSensorBreakMenuItem *item, int index) {
    if (!is_obj_valid(cont)) {
        ESP_LOGE(TAG, "Invalid container in create_gvs_sensor_break_menu_item");
        return;
    }
    
    lv_obj_t *box = lv_obj_create(cont);
    if (!is_obj_valid(box)) {
        ESP_LOGE(TAG, "Failed to create box in create_gvs_sensor_break_menu_item");
        return;
    }
...
void gvs_sensor_break_menu_show(void) {
    ESP_LOGI(TAG, "Showing GVS sensor break menu");
    if (is_obj_valid(gvs_sensor_break_cont)) {
        lv_obj_clear_flag(gvs_sensor_break_cont, LV_OBJ_FLAG_HIDDEN);
    }
    if (!is_obj_valid(gvs_sensor_break_mask)) {
        gvs_sensor_break_mask = radial();
        if (is_obj_valid(gvs_sensor_break_mask)) {
            lv_obj_set_pos(gvs_sensor_break_mask, 433, 70);
        }
    } else {
        lv_obj_clear_flag(gvs_sensor_break_mask, LV_OBJ_FLAG_HIDDEN);
    }
}

void gvs_sensor_break_menu_hide(void) {
    ESP_LOGI(TAG, "Hiding GVS sensor break menu");
    if (is_obj_valid(gvs_sensor_break_cont)) {
        lv_obj_add_flag(gvs_sensor_break_cont, LV_OBJ_FLAG_HIDDEN);
    }
    if (is_obj_valid(gvs_sensor_break_mask)) {
        lv_obj_del(gvs_sensor_break_mask);
        gvs_sensor_break_mask = NULL;
    }
}
...
void gvs_sensor_break_menu_encoder_event_cb(uint8_t e) {
    ...
    if (!is_obj_valid(gvs_sensor_break_cont)) {
        ESP_LOGE(TAG, "Контейнер меню обрыва датчика ГВС не инициализирован");
        return;
    }
    
    if (edit_mode && editing_param_index >= 0) {
        if (e & ENC_LEFT) {
            int step = gvs_sensor_break_param_limits_int[editing_param_index].step;
            editing_int_value -= step;
            if (editing_int_value < gvs_sensor_break_param_limits_int[editing_param_index].min) {
                // Для enum параметров делаем циклическое переключение
                if (editing_param_index == 0 || editing_param_index == 2) {
                    editing_int_value = gvs_sensor_break_param_limits_int[editing_param_index].max;
                } else {
                    editing_int_value = gvs_sensor_break_param_limits_int[editing_param_index].min;
                }
            }
            ...
            int step = gvs_sensor_break_param_limits_int[editing_param_index].step;
            editing_int_value += step;
            if (editing_int_value > gvs_sensor_break_param_limits_int[editing_param_index].max) {
                // Для enum параметров делаем циклическое переключение
                if (editing_param_index == 0 || editing_param_index == 2) {
                    editing_int_value = gvs_sensor_break_param_limits_int[editing_param_index].min;
                } else {
    ...
    arc_menu_handle_encoder(e, gvs_sensor_break_cont, menu_state, MENU_TYPE_GVS_ALARMS_SENSOR_BREAK);
    ...
    if (e & ENC_CLICK) {
        if (menu_state->cursor_index == 0) {
            ESP_LOGI(TAG, "Returning to GVS alarms menu from sensor break menu");
            gvs_sensor_break_menu_hide();
            gvs_alarms_menu_show();
            encoder_manager_register_callback(gvs_alarms_menu_encoder_event_cb);
        } else {
            int param_index = gvs_sensor_break_menu_items[menu_state->cursor_index].param_index;
            if (param_index >= 0) {
...
void gvs_sensor_break_menu_cleanup(void) {
    ESP_LOGI(TAG, "Cleaning up GVS sensor break menu");
    
    gvs_sensor_break_menu_creation_in_progress = false;
    edit_mode = false;
    editing_param_index = -1;
    ...
    if (is_obj_valid(gvs_sensor_break_cont)) {
        lv_obj_del(gvs_sensor_break_cont);
        gvs_sensor_break_cont = NULL;
    }
    
    gvs_sensor_break_menu_initialized = false;
}

void GVS_Sensor_Break_Menu_List(void) {
    ESP_LOGI(TAG, "Инициализация меню обрыва датчика ГВС");
    
    if (gvs_sensor_break_menu_creation_in_progress) {
        ESP_LOGW(TAG, "GVS sensor break menu creation already in progress, skipping");
        return;
    }
    
    gvs_sensor_break_menu_creation_in_progress = true;
    
    if (gvs_sensor_break_menu_initialized && is_obj_valid(gvs_sensor_break_cont)) {
        ESP_LOGI(TAG, "GVS sensor break menu already initialized, showing it");
        gvs_sensor_break_menu_show();
        gvs_sensor_break_menu_creation_in_progress = false;
        return;
    }
    
    gvs_sensor_break_menu_cleanup();
    ...
    gvs_sensor_break_cont = lv_obj_create(lv_scr_act());
    if (!is_obj_valid(gvs_sensor_break_cont)) {
        ESP_LOGE(TAG, "Failed to create GVS sensor break menu container");
        gvs_sensor_break_menu_creation_in_progress = false;
        return;
    }
    
    lv_obj_set_size(gvs_sensor_break_cont, 1200, 1200);
    lv_obj_center(gvs_sensor_break_cont);
    lv_obj_add_event_cb(gvs_sensor_break_cont, arc_menu_event_cb, LV_EVENT_SCROLL, NULL);
    lv_obj_add_style(gvs_sensor_break_cont, &style, 0);
    lv_obj_set_style_radius(gvs_sensor_break_cont, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_clip_corner(gvs_sensor_break_cont, true, 0);
    lv_obj_set_scroll_dir(gvs_sensor_break_cont, LV_DIR_VER);
    lv_obj_set_scroll_snap_y(gvs_sensor_break_cont, LV_SCROLL_SNAP_CENTER);
    lv_obj_set_flex_flow(gvs_sensor_break_cont, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_pos(gvs_sensor_break_cont, 633, 0);
    lv_obj_set_style_bg_color(gvs_sensor_break_cont, lv_color_hex(0x1E2528), 0);
    lv_obj_set_style_border_color(gvs_sensor_break_cont, lv_color_hex(0x1E2528), 0);
    lv_obj_set_style_shadow_width(gvs_sensor_break_cont, 0, 0);
    lv_obj_set_style_pad_row(gvs_sensor_break_cont, 1, 0);
    
    vTaskDelay(pdMS_TO_TICKS(10));
    
    for (uint32_t i = 0; i < sizeof(gvs_sensor_break_menu_items) / sizeof(GvsSensorBreakMenuItem); i++) {
        create_gvs_sensor_break_menu_item(gvs_sensor_break_cont, &gvs_sensor_break_menu_items[i], i);
        if (i % 3 == 0) {
    ...
    const menu_config_t* config = get_menu_config(MENU_TYPE_GVS_ALARMS_SENSOR_BREAK);
    menu_state_t *menu_state = get_menu_state(MENU_TYPE_GVS_ALARMS_SENSOR_BREAK);
    
    menu_state->list_index = config->initial_index;
    menu_state->cursor_index = 0;
    
    lv_obj_t *target_child = lv_obj_get_child(gvs_sensor_break_cont, menu_state->list_index);
    if (target_child) {
        lv_obj_scroll_to_view(target_child, LV_ANIM_OFF);
    }
    
    gvs_sensor_break_highlight_box(gvs_sensor_break_cont, menu_state->cursor_index);
    arc_menu_update_slide(gvs_sensor_break_cont);
    
    gvs_sensor_break_menu_initialized = true;
    gvs_sensor_break_menu_creation_in_progress = false;

    ESP_LOGI(TAG, "Меню обрыва датчика ГВС успешно инициализировано");
}
