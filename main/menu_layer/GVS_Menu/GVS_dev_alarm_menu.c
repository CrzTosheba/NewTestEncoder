...
static const char *TAG = "GVS_DEV_ALARM_MENU";
...
static const GvsDevAlarmMenuItem gvs_dev_alarm_menu_items[] = {
    {"                                Назад", &lv_im_arrow_right, -1},
    {"Авар.откл.Тпод_ГВС", NULL, 0},          // GVS_T1_EnDevAlarm (enum)
    {"Перегрев Тпод_ГВС", NULL, 1},             // GVS_T1_EnHighAlarm (enum)
    {"Недогрев Тпод_ГВС", NULL, 2},             // GVS_T1_EnLowAlarm (enum)
    {"Задержка, с", NULL, 3},                  // GVS_T1_DevAlarmDelay (int)
    {"Сброс", NULL, 4},                        // GVS_T1_DevAlarmRType (enum)
    {"Макс.откл.Тпод_ГВС, °C", NULL, 5},       // GVS_T1_AlarmDev (float)
};

lv_obj_t *gvs_dev_alarm_cont = NULL;
static bool gvs_dev_alarm_menu_initialized = false;
static bool gvs_dev_alarm_menu_creation_in_progress = false;
static lv_obj_t *gvs_dev_alarm_mask = NULL;
...
static int temp_GVS_T1_EnDevAlarm = 0;
static int temp_GVS_T1_EnHighAlarm = 0;
static int temp_GVS_T1_EnLowAlarm = 0;
static int temp_GVS_T1_DevAlarmDelay = 0;
static int temp_GVS_T1_DevAlarmRType = 0;
static float temp_GVS_T1_AlarmDev = 0.0f;
...
static void gvs_dev_alarm_highlight_box(lv_obj_t *cont, uint32_t cursor_index) {
    ...
    for (uint32_t i = 0; i < child_cnt; i++) {
        ...
        bool is_editing_this = (edit_mode && editing_param_index == gvs_dev_alarm_menu_items[i].param_index);
...
static void update_param_display(int param_index) {
    ...
    if (edit_mode && editing_param_index == param_index) {
        ...
        switch(param_index) {
            case 0: {
                int value = GVS_T1_EnDevAlarm;
                snprintf(value_str, sizeof(value_str), "%s", value == 0 ? "НЕТ" : "ДА");
                break;
            }
            case 1: {
                int value = GVS_T1_EnHighAlarm;
                snprintf(value_str, sizeof(value_str), "%s", value == 0 ? "НЕТ" : "ДА");
                break;
            }
            case 2: {
                int value = GVS_T1_EnLowAlarm;
                snprintf(value_str, sizeof(value_str), "%s", value == 0 ? "НЕТ" : "ДА");
                break;
            }
            case 3: snprintf(value_str, sizeof(value_str), "%d", GVS_T1_DevAlarmDelay); break;
            case 4: {
                snprintf(value_str, sizeof(value_str), "%s", get_dev_alarm_rtype_string(GVS_T1_DevAlarmRType));
                break;
            }
            case 5: format_float_value(value_str, sizeof(value_str), GVS_T1_AlarmDev); break;
        }
    }
...
static void save_param_changes(void) {
    ...
    switch(editing_param_index) {
        case 0: GVS_T1_EnDevAlarm = editing_int_value; break;
        case 1: GVS_T1_EnHighAlarm = editing_int_value; break;
        case 2: GVS_T1_EnLowAlarm = editing_int_value; break;
        case 3: GVS_T1_DevAlarmDelay = editing_int_value; break;
        case 4: GVS_T1_DevAlarmRType = editing_int_value; break;
        case 5: GVS_T1_AlarmDev = editing_float_value; break;
    }
    ...
    menu_state_t *menu_state = get_menu_state(MENU_TYPE_GVS_ALARMS_DEVIATION);
    if (menu_state) {
        gvs_dev_alarm_highlight_box(gvs_dev_alarm_cont, menu_state->cursor_index);
    }
}
...
static void cancel_param_changes(void) {
    ...
    switch(editing_param_index) {
        case 0: editing_int_value = temp_GVS_T1_EnDevAlarm; break;
        case 1: editing_int_value = temp_GVS_T1_EnHighAlarm; break;
        case 2: editing_int_value = temp_GVS_T1_EnLowAlarm; break;
        case 3: editing_int_value = temp_GVS_T1_DevAlarmDelay; break;
        case 4: editing_int_value = temp_GVS_T1_DevAlarmRType; break;
        case 5: editing_float_value = temp_GVS_T1_AlarmDev; break;
    }
...
static void enter_edit_mode(int param_index) {
    ...
    switch(param_index) {
        case 0:
            temp_GVS_T1_EnDevAlarm = GVS_T1_EnDevAlarm;
            editing_int_value = GVS_T1_EnDevAlarm;
            break;
        case 1:
            temp_GVS_T1_EnHighAlarm = GVS_T1_EnHighAlarm;
            editing_int_value = GVS_T1_EnHighAlarm;
            break;
        case 2:
            temp_GVS_T1_EnLowAlarm = GVS_T1_EnLowAlarm;
            editing_int_value = GVS_T1_EnLowAlarm;
            break;
        case 3:
            temp_GVS_T1_DevAlarmDelay = GVS_T1_DevAlarmDelay;
            editing_int_value = GVS_T1_DevAlarmDelay;
            break;
        case 4:
            temp_GVS_T1_DevAlarmRType = GVS_T1_DevAlarmRType;
            editing_int_value = GVS_T1_DevAlarmRType;
            break;
        case 5:
            temp_GVS_T1_AlarmDev = GVS_T1_AlarmDev;
            editing_float_value = GVS_T1_AlarmDev;
            break;
    }
...
static void exit_edit_mode_with_confirmation(void) {
    ...
    if (editing_param_index == 5) {
        value_changed = (fabs(editing_float_value - temp_GVS_T1_AlarmDev) > 0.01f);
    } else {
        switch(editing_param_index) {
            case 0: value_changed = (editing_int_value != temp_GVS_T1_EnDevAlarm); break;
            case 1: value_changed = (editing_int_value != temp_GVS_T1_EnHighAlarm); break;
            case 2: value_changed = (editing_int_value != temp_GVS_T1_EnLowAlarm); break;
            case 3: value_changed = (editing_int_value != temp_GVS_T1_DevAlarmDelay); break;
            case 4: value_changed = (editing_int_value != temp_GVS_T1_DevAlarmRType); break;
        }
    }
...
static void create_gvs_dev_alarm_menu_item(lv_obj_t *cont, const GvsDevAlarmMenuItem *item, int index) {
    if (!is_obj_valid(cont)) {
        ESP_LOGE(TAG, "Invalid container in create_gvs_dev_alarm_menu_item");
        return;
    }
    
    lv_obj_t *box = lv_obj_create(cont);
    if (!is_obj_valid(box)) {
        ESP_LOGE(TAG, "Failed to create box in create_gvs_dev_alarm_menu_item");
        return;
    }
...
void gvs_dev_alarm_menu_show(void) {
    ESP_LOGI(TAG, "Showing GVS deviation alarm menu");
    if (is_obj_valid(gvs_dev_alarm_cont)) {
        lv_obj_clear_flag(gvs_dev_alarm_cont, LV_OBJ_FLAG_HIDDEN);
    }
    if (!is_obj_valid(gvs_dev_alarm_mask)) {
        gvs_dev_alarm_mask = radial();
        if (is_obj_valid(gvs_dev_alarm_mask)) {
            lv_obj_set_pos(gvs_dev_alarm_mask, 433, 70);
        }
    } else {
        lv_obj_clear_flag(gvs_dev_alarm_mask, LV_OBJ_FLAG_HIDDEN);
    }
}

void gvs_dev_alarm_menu_hide(void) {
    ESP_LOGI(TAG, "Hiding GVS deviation alarm menu");
    if (is_obj_valid(gvs_dev_alarm_cont)) {
        lv_obj_add_flag(gvs_dev_alarm_cont, LV_OBJ_FLAG_HIDDEN);
    }
    if (is_obj_valid(gvs_dev_alarm_mask)) {
        lv_obj_del(gvs_dev_alarm_mask);
        gvs_dev_alarm_mask = NULL;
    }
}
...
void gvs_dev_alarm_menu_encoder_event_cb(uint8_t e) {
    ...
    if (!is_obj_valid(gvs_dev_alarm_cont)) {
        ESP_LOGE(TAG, "Контейнер меню аварийного отклонения ГВС не инициализирован");
        return;
    }
    ...
    if (edit_mode && editing_param_index >= 0) {
        if (e & ENC_LEFT) {
            if (editing_param_index == 5) {
                float step = gvs_dev_alarm_param_limits_float[0].step;
                editing_float_value -= step;
                if (editing_float_value < gvs_dev_alarm_param_limits_float[0].min) {
                    editing_float_value = gvs_dev_alarm_param_limits_float[0].min;
                }
            } else {
                int step = gvs_dev_alarm_param_limits_int[editing_param_index].step;
                editing_int_value -= step;
                if (editing_int_value < gvs_dev_alarm_param_limits_int[editing_param_index].min) {
                    ...
                    if (editing_param_index == 0 || editing_param_index == 1 || editing_param_index == 2 || editing_param_index == 4) {
                        editing_int_value = gvs_dev_alarm_param_limits_int[editing_param_index].max;
                    } else {
                        editing_int_value = gvs_dev_alarm_param_limits_int[editing_param_index].min;
                    }
                }
            }
            ...
            if (editing_param_index == 5) {
                float step = gvs_dev_alarm_param_limits_float[0].step;
                editing_float_value += step;
                if (editing_float_value > gvs_dev_alarm_param_limits_float[0].max) {
                    editing_float_value = gvs_dev_alarm_param_limits_float[0].max;
                }
            } else {
                int step = gvs_dev_alarm_param_limits_int[editing_param_index].step;
                editing_int_value += step;
                if (editing_int_value > gvs_dev_alarm_param_limits_int[editing_param_index].max) {
                    // Для enum параметров делаем циклическое переключение
                    if (editing_param_index == 0 || editing_param_index == 1 || editing_param_index == 2 || editing_param_index == 4) {
                        editing_int_value = gvs_dev_alarm_param_limits_int[editing_param_index].min;
                    } else {
    ...
    arc_menu_handle_encoder(e, gvs_dev_alarm_cont, menu_state, MENU_TYPE_GVS_ALARMS_DEVIATION);
    ...
    if (e & ENC_CLICK) {
        if (menu_state->cursor_index == 0) {
            ...
            gvs_dev_alarm_menu_hide();
            gvs_alarms_menu_show();
            encoder_manager_register_callback(gvs_alarms_menu_encoder_event_cb);
        } else {
            int param_index = gvs_dev_alarm_menu_items[menu_state->cursor_index].param_index;
            if (param_index >= 0) {
...
void gvs_dev_alarm_menu_cleanup(void) {
    ESP_LOGI(TAG, "Cleaning up GVS deviation alarm menu");
    
    gvs_dev_alarm_menu_creation_in_progress = false;
    edit_mode = false;
    editing_param_index = -1;
    ...
    if (is_obj_valid(gvs_dev_alarm_cont)) {
        lv_obj_del(gvs_dev_alarm_cont);
        gvs_dev_alarm_cont = NULL;
    }
    
    gvs_dev_alarm_menu_initialized = false;
}

void GVS_Dev_Alarm_Menu_List(void) {
    ESP_LOGI(TAG, "Инициализация меню аварийного отклонения ГВС");
    
    if (gvs_dev_alarm_menu_creation_in_progress) {
        ESP_LOGW(TAG, "GVS deviation alarm menu creation already in progress, skipping");
        return;
    }
    
    gvs_dev_alarm_menu_creation_in_progress = true;
    
    if (gvs_dev_alarm_menu_initialized && is_obj_valid(gvs_dev_alarm_cont)) {
        ESP_LOGI(TAG, "GVS deviation alarm menu already initialized, showing it");
        gvs_dev_alarm_menu_show();
        gvs_dev_alarm_menu_creation_in_progress = false;
        return;
    }
    
    gvs_dev_alarm_menu_cleanup();
    ...
    gvs_dev_alarm_cont = lv_obj_create(lv_scr_act());
    if (!is_obj_valid(gvs_dev_alarm_cont)) {
        ESP_LOGE(TAG, "Failed to create GVS deviation alarm menu container");
        gvs_dev_alarm_menu_creation_in_progress = false;
        return;
    }
    
    lv_obj_set_size(gvs_dev_alarm_cont, 1200, 1200);
    lv_obj_center(gvs_dev_alarm_cont);
    lv_obj_add_event_cb(gvs_dev_alarm_cont, arc_menu_event_cb, LV_EVENT_SCROLL, NULL);
    lv_obj_add_style(gvs_dev_alarm_cont, &style, 0);
    lv_obj_set_style_radius(gvs_dev_alarm_cont, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_clip_corner(gvs_dev_alarm_cont, true, 0);
    lv_obj_set_scroll_dir(gvs_dev_alarm_cont, LV_DIR_VER);
    lv_obj_set_scroll_snap_y(gvs_dev_alarm_cont, LV_SCROLL_SNAP_CENTER);
    lv_obj_set_flex_flow(gvs_dev_alarm_cont, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_pos(gvs_dev_alarm_cont, 633, 0);
    lv_obj_set_style_bg_color(gvs_dev_alarm_cont, lv_color_hex(0x1E2528), 0);
    lv_obj_set_style_border_color(gvs_dev_alarm_cont, lv_color_hex(0x1E2528), 0);
    lv_obj_set_style_shadow_width(gvs_dev_alarm_cont, 0, 0);
    lv_obj_set_style_pad_row(gvs_dev_alarm_cont, 1, 0);
    
    vTaskDelay(pdMS_TO_TICKS(10));
    
    for (uint32_t i = 0; i < sizeof(gvs_dev_alarm_menu_items) / sizeof(GvsDevAlarmMenuItem); i++) {
        create_gvs_dev_alarm_menu_item(gvs_dev_alarm_cont, &gvs_dev_alarm_menu_items[i], i);
        if (i % 3 == 0) {
    ...
    const menu_config_t* config = get_menu_config(MENU_TYPE_GVS_ALARMS_DEVIATION);
    menu_state_t *menu_state = get_menu_state(MENU_TYPE_GVS_ALARMS_DEVIATION);
    
    menu_state->list_index = config->initial_index;
    menu_state->cursor_index = 0;
    
    lv_obj_t *target_child = lv_obj_get_child(gvs_dev_alarm_cont, menu_state->list_index);
    if (target_child) {
        lv_obj_scroll_to_view(target_child, LV_ANIM_OFF);
    }
    
    gvs_dev_alarm_highlight_box(gvs_dev_alarm_cont, menu_state->cursor_index);
    arc_menu_update_slide(gvs_dev_alarm_cont);
    
    gvs_dev_alarm_menu_initialized = true;
    gvs_dev_alarm_menu_creation_in_progress = false;

    ESP_LOGI(TAG, "Меню аварийного отклонения ГВС успешно инициализировано");
}
