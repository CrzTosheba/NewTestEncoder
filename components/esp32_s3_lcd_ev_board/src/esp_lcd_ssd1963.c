...
#define SSD1963_COL   (480)
#define SSD1963_ROW   (800)

#define SSD1963_HDP   (SSD1963_COL - 1) // Hsync Display period   
#define SSD1963_VDP   (SSD1963_ROW - 1) // Vertical Display period

#define SSD1963_HBP   (20)    // Horizontal Back porch pixels
#define SSD1963_HFP   (16)    // Horizontal Front porch pixels
#define SSD1963_HPW   (10)    // Horizontal Sync Pulse Width = (HPW + 1) pixels	
#define SSD1963_HPS   (SSD1963_HPW + SSD1963_HBP)    // Horizontal Sync pulse width + horizontal back porch
#define SSD1963_HT    (SSD1963_COL + SSD1963_HBP + SSD1963_HFP + SSD1963_HPW)   // Horizontal total period = (HT + 1) pixels

#define SSD1963_VBP   (16)    // Vertical back porch
#define SSD1963_VFP   (14)    // Vertical front porch
#define SSD1963_VPW   (4)     // Vertical Sync Pulse Width = (VPW + 1) lines
#define SSD1963_VPS   (SSD1963_VPW + SSD1963_VBP) // Vertical pulse width + vertical back porch
#define SSD1963_VT    (834)   // Vertical Total = (VT + 1) lines

#define SSD1963_PLL_M (49)
#define SSD1963_PLL_N (3)

static const ssd1963_lcd_init_cmd_t vendor_specific_init_default[] = {
//  {cmd, { data }, data_size, delay_ms}
    {0x01, (uint8_t []){0x00}, 1, 50},

    {0xE2, (uint8_t []){SSD1963_PLL_M, SSD1963_PLL_N, 0x54}, 3, 0},
	
    {0xE0, (uint8_t []){0x01}, 1, 5},
    {0xE0, (uint8_t []){0x03}, 1, 5},

    {0xE6, (uint8_t []){0x01, 0x99, 0x99}, 3, 0}, // PCLK = 10 MHz
	
//    {0xB0, (uint8_t []){0x00, 0x00, (((480-1) >> 8) & 0xff), ((480-1) & 0xff), (((800-1) >> 8) & 0xff), ((800-1) & 0xff), 0x00}, 7, 5},
    {0xB0, (uint8_t []){0x00, 0x00, ((SSD1963_HDP >> 8) & 0xff), (SSD1963_HDP & 0xff), ((SSD1963_VDP >> 8) & 0xff), (SSD1963_VDP & 0xff), 0x00}, 7, 5},
	
//    {0xB4, (uint8_t []){(((526-1) >> 8) & 0xff), ((526-1) & 0xff), (((30) >> 8) & 0xff), ((30) & 0xff), 10-1, ((0 >> 8) & 0xff), (0 & 0xff), 0x00}, 8, 0},
    {0xB4, (uint8_t []){(((SSD1963_HT-1) >> 8) & 0xff), ((SSD1963_HT-1) & 0xff), (((SSD1963_HPS) >> 8) & 0xff), ((SSD1963_HPS) & 0xff), SSD1963_HPW-1, 0, 0, 0x00}, 8, 0},
	
//    {0xB6, (uint8_t []){(((834-1) >> 8) & 0xff), ((834-1) & 0xff), (((20-1) >> 8) & 0xff), ((20-1) & 0xff), 4-1, ((0 >> 8) & 0xff), (0 & 0xff)}, 7, 0},
    {0xB6, (uint8_t []){(((SSD1963_VT-1) >> 8) & 0xff), ((SSD1963_VT-1) & 0xff), (((SSD1963_VPS-1) >> 8) & 0xff), ((SSD1963_VPS-1) & 0xff), SSD1963_VPW-1, 0, 0}, 7, 0},

//    {0x36, (uint8_t []){0x00}, 1, 0}, // normal
    {0x36, (uint8_t []){ ( 1<<5 ) }, 1, 0}, // swap X,Y

    {0xF0, (uint8_t []){0x03}, 1, 5}, // 011 16-bit (565 format)

//    {0xBE, (uint8_t []){0x06, 0x80, 0x01, 0xff, 0x00, 0x00}, 6, 0},
    {0xBE, (uint8_t []){0x06, 0xff, 0x01, 0x00, 0x00, 0x00}, 6, 0}, // [1]-ff brightest MAX, PWM(0x80) = 0.5, PWM(0x00) = 0
...
/*		
		printf("init cmd(0x%x): ", init_cmds[i].cmd);
		for(uint8_t j = 0; j < init_cmds[i].data_bytes; j++) {
			uint8_t *t = (uint8_t*)init_cmds[i].data;
			printf(" 0x%02x", *(t + j));
		}
		printf("\n");
*/
...
static esp_err_t panel_ssd1963_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
{
    ...
    if (on_off) {
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0xBE,
        (uint8_t []){0x06, 0xff, 0x01, 0x00, 0x00, 0x00}, 6), TAG, "send command failed");

        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_DISPON, NULL, 0), TAG, "send command failed");
    } else {
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0xBE,
        (uint8_t []){0x06, 0x00, 0x01, 0x00, 0x00, 0x00}, 6), TAG, "send command failed");

        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_DISPOFF, NULL, 0), TAG, "send command failed");
    }
